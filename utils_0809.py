...
def cxcy_to_gcxgcy(cxcy, priors_cxcy):
    ...
    w=priors_cxcy[:, 2].clone()
    h=priors_cxcy[:, 3].clone()
    
    eps=torch.FloatTensor([1e-6]).expand_as(w)
    ...
    img_size=torch.cat([w.unsqueeze(1),h.unsqueeze(1)],1)
    
#     return torch.cat([((cxcy[:, :2] - priors_cxcy[:, :2]))/(priors_cxcy[:, 2:]) ,  # g_c_x, g_c_y
#                       torch.log(cxcy[:, 2:] / (priors_cxcy[:, 2:])) ], 1)  # g_w, g_h
    return torch.cat([((cxcy[:, :2] - priors_cxcy[:, :2]))/(img_size) ,  # g_c_x, g_c_y
                      torch.log(cxcy[:, 2:] / (img_size)) ], 1)  # g_w, g_h
...
def resize(image, boxes,return_percent_coords=True):
    """
...
#     new_boxes = (boxes / old_dims)*new_dims  # percent coordinates
    
    if not return_percent_coords:
        new_dims = torch.FloatTensor([dims[1], dims[0], dims[1], dims[0]]).unsqueeze(0)
        new_boxes = new_boxes * new_dims

    return new_image, new_boxes,scale
...
def transform(image, boxes, labels, difficulties, split):
    ...
    new_image, new_boxes,scale = resize(new_image, new_boxes,False)
    ...
    return new_image, new_boxes, new_labels, new_difficulties,scale
